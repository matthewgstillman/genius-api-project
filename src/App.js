...
const Home = () => {
  ...
  const fetchPublicData = async () => {
    try {
      const response = await axios.get('http://localhost:8080/https://api.genius.com/account', {
        headers: {
    ...
    const handleToken = async () => {
      ...
      try {
        const response = await axios.post("http://localhost:8080/https://api.genius.com/oauth/token", {
        ...
        console.error("Error exchanging code for token:", error.response || error.message);
    ...
    <div className="App">
      <p>click this, itll redirect to the login page for genuis</p>
        ...
        <div>
          ...
          <p>The url has the code?  Okay, watch in the dev tools 'network' panel and click this button.  </p>
          <button onClick={handleToken}>Use that token</button>
          <p>Did token endpoint return the access token?  Great, we can make a request now:</p>
